...
fs.readdirSync(attachmentsDir).forEach(file => {
  ...
  if (fs.statSync(filePath).isFile() && path.extname(file).toLowerCase() === '.png' && file.startsWith('icons_odoo_')) {
    ...
    console.log(`Skipping non-PNG file or file without 'icons_odoo_' prefix: ${file}`);
  }
});